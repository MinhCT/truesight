# AGENTS.md - AI Agent Development Guide for tap-youtube-persona
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from YoutubePersona.

### Key Components

1. **Tap Class** (`tap_youtube_persona/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_youtube_persona/client.py`): Handles API communication and authentication
1. **Streams** (`tap_youtube_persona/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_youtube_persona/streams.py`
...
```python
class MyNewStream(YoutubePersonaStream):
    name = "my_new_stream"
...
1. **SimpleHeaderPaginator**: For APIs using Link headers (RFC 5988)
   ...
   class MyStream(YoutubePersonaStream):
...
```bash
tap-youtube-persona --config config.json --discover
tap-youtube-persona --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap_youtube_persona/tap.py`
...
```bash
# .env.example
TAP_YOUTUBEPERSONA_API_URL=https://api.example.com
TAP_YOUTUBEPERSONA_API_KEY=your_api_key_here
TAP_YOUTUBEPERSONA_BATCH_SIZE=100  # New setting
```
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-youtube-persona --version`), Python version, and a minimal reproduction case when reporting bugs.
